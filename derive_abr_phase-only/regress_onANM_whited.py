...
for subject in subject_list:
    ...
    w_anm = np.zeros((n_stim, len_eeg))
    abr_anm = np.zeros((n_stim, 14000))
    ##### For unaltered response
    print("unaltered sub{:02}".format(subject))


    stim_data = read_hdf5(ANM_path + 'unaltered_x_in.hdf5')
    # Load x_in
    ...
    # TRF
    #denom_pos = np.mean(x_in_pos_fft * np.conj(x_in_pos_fft), axis=0) DO NOT NEED DENOMINATORS
    ...
    for ei in range(n_epoch):
        ...
        w_pos += [w_i_pos]
        w_neg += [w_i_neg]
    w_anm[0] = (ifft(np.array(w_pos).sum(0)).real +
                   ifft(np.array(w_neg).sum(0)).real) / 2

    abr_anm[0] = np.roll(np.concatenate((w_anm[0][int(t_start*eeg_fs):],
                            w_anm[0][0:int(t_stop*eeg_fs)])),
                            int(3.4*eeg_fs/1000))
    
    ##### For broadband peaky response
    print("broadband peaky sub{:02}".format(subject))
    
    stim_data = read_hdf5(ANM_path + 'broadband_x_in.hdf5')
    # Load x_in
    ...
    for ei in range(n_epoch):
        ...
        w_pos += [w_i_pos]
        w_neg += [w_i_neg]
    w_anm[1] = (ifft(np.array(w_pos).sum(0)).real +
                    ifft(np.array(w_neg).sum(0)).real) / 2

    abr_anm[1] = np.roll(np.concatenate((w_anm[1][int(t_start*eeg_fs):],
                             w_anm[1][0:int(t_stop*eeg_fs)])),
                             int(3.4*eeg_fs/1000))
    
    write_hdf5(save_path + 'ANM_response/' + eeg_root.format(subject, task) +
               '_ANM_whited_responses.hdf5',
               dict(w_anm=w_anm,
                    abr_anm=abr_anm,
