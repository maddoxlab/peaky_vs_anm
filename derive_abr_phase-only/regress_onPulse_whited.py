...
pulse_path = opath + '/present_files/pulse/'
eeg_root = 'sub-{:02}_task-{}'
...
for subject in subject_list:
    ...
    w_pulse = np.zeros((n_stim, len_eeg))
    abr_pulse = np.zeros((n_stim, 14000))
    ##### For unaltered response
    print("unaltered sub{:02}".format(subject))


    stim_data = read_hdf5(pulse_path + 'unaltered_x_in.hdf5')
    # Load x_in
    x_in_pos = stim_data['x_in_unaltered']
    x_in_neg = stim_data['x_in_unaltered']
    del stim_data
    # Load x_out
    ...
    x_in_pos_fft = x_in_pos_fft/np.abs(x_in_pos_fft)
    x_in_neg_fft = fft(x_in_neg)
    x_in_neg_fft = x_in_neg_fft/np.abs(x_in_neg_fft)
    # x_out fft
    x_out_fft = fft(x_out)
    if Bayesian:
    ...
    for ei in range(n_epoch):
        ...
        w_pos += [w_i_pos]
        w_neg += [w_i_neg]
    w_pulse[0] = (ifft(np.array(w_pos).sum(0)).real +
                   ifft(np.array(w_neg).sum(0)).real) / 2

    abr_pulse[0] = np.concatenate((w_pulse[0][int(t_start*eeg_fs):],
                            w_pulse[0][0:int(t_stop*eeg_fs)]))
    
    ##### For broadband peaky response
    print("broadband peaky sub{:02}".format(subject))
    
    stim_data = read_hdf5(pulse_path + 'broadband_x_in.hdf5')
    # Load x_in
    x_in_pos = stim_data['x_in_broadband']
    x_in_neg = stim_data['x_in_broadband']
    # Load x_out
    ...
    for ei in range(n_epoch):
        ...
        w_pos += [w_i_pos]
        w_neg += [w_i_neg]
    w_pulse[1] = (ifft(np.array(w_pos).sum(0)).real +
                    ifft(np.array(w_neg).sum(0)).real) / 2

    abr_pulse[1] = np.concatenate((w_pulse[1][int(t_start*eeg_fs):],
                             w_pulse[1][0:int(t_stop*eeg_fs)]))
    
    write_hdf5(save_path + 'pulse_response/' + eeg_root.format(subject, task) +
               '_pulse_whited_responses.hdf5',
               dict(w_pulse=w_pulse,
                    abr_pulse=abr_pulse,
